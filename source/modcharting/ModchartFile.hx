...
class ModchartFile {
	...
	public function new(renderer:PlayfieldRenderer) {
		...
		data = loadFromJson(PlayState.SONG.song.toLowerCase(),
			Difficulty.getString().toLowerCase() == null ? Difficulty.defaultList[PlayState.storyDifficulty] : Difficulty.getString().toLowerCase());
		#elseif (PSYCHVERSION < "0.7")
		data = loadFromJson(PlayState.SONG.song.toLowerCase(),
			CoolUtil.difficultyString().toLowerCase() == null ? CoolUtil.difficulties[PlayState.storyDifficulty] : CoolUtil.difficultyString().toLowerCase());
		#end
		#else
	...
	public function loadFromJson(folder:String, difficulty:String):ModchartJson // load da shit
	{
		...
		#if MODS_ALLOWED
		var moddyFileMods:String = Paths.modsJson(#if PSYCH Paths.formatToSongPath(folder) #else PlayState.SONG.song #end
			+ '/modchart-'
		...
		try {
			...
			else if (FileSystem.exists(moddyFileMods2) && difficulty.toLowerCase() == null && !FileSystem.exists(moddyFileMods))
				hasDifficultyModchart = false;
			#end
			...
			if (FileSystem.exists(moddyFile) && !FileSystem.exists(moddyFile))
				hasDifficultyModchart = false;
			else if (FileSystem.exists(moddyFile2) && difficulty.toLowerCase() == null && !FileSystem.exists(moddyFile))
				hasDifficultyModchart = false;
			...
			if (Assets.exists(moddyFileMods2) && !Assets.exists(moddyFileMods))
				hasDifficultyModchart = false;
			else if (Assets.exists(moddyFileMods2) && difficulty.toLowerCase() == null && !Assets.exists(moddyFileMods))
				hasDifficultyModchart = false;
			#end
			...
			if (Assets.exists(moddyFile) && !Assets.exists(moddyFile))
				hasDifficultyModchart = false;
			else if (Assets.exists(moddyFile2) && difficulty.toLowerCase() == null && !Assets.exists(moddyFile))
				hasDifficultyModchart = false;
		...
		if (rawJson == null) {
			try {
				...
				if (hasDifficultyModchart) {
					#if LEATHER
					filePath = folderShit = PolymodAssets.getPath(filePath.replace('modchart-' + difficulty.toLowerCase() + '.json', "customMods/"));
					#else
		...
		try {
				...
				if (FileSystem.isDirectory(folderShit)) {
					...
					for (file in FileSystem.readDirectory(folderShit)) {
						...
						if (file.endsWith('.hx')) // custom mods!!!!
						{
							...
							scriptInit = #if (HSCRIPT_ALLOWED && PSYCH && PSYCHVERSION >= "0.7") new FunkinHScript(null,
								scriptStr) #else new CustomModifierScript(scriptStr) #end;
			...
			return cast {modifiers: [], events: [], playfields: 1};
		}
		return json;
	}
